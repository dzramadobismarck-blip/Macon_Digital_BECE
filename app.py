...
if search_active:
    if not filtered_df.empty:
        ...
        # User selection for number of questions
        limit = st.number_input("Number of questions to quiz on:", min_value=1, max_value=len(filtered_df), value=min(5, len(filtered_df)))
        
        if st.button("Take Quiz with these Results"):
            st.session_state.quiz_data = filtered_df.sample(limit)
...
num_rand = st.number_input("Number of random questions:", min_value=1, max_value=20, value=5)
if st.button("Start Random Quiz"):
    st.session_state.quiz_data = df.sample(min(num_rand, len(df)))
    st.session_state.submitted = False
    st.session_state.user_answers = {}
    st.rerun()
